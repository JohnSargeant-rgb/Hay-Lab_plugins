	...
	for image in list_dir:
		if image.endswith(file_def):
			...
			if image.startswith(ext_def):
				processed +=1
	percent_complete =(processed/total_images)*100
...
gdp.addMessage(str(progress[0])+'% Images Assayed' +' '+ '('+ str(progress[2]) +'/' +str(progress[1]) +')', italicFont)
...
while exit_loop ==0:
	...
	headers = ['Cell', 'AVG_Max', "bck", "ER1", "ER2", "ER3", "net_ER", "net_golgi", "T_Index", "Image_title"]
	with open(Quant, 'ab') as f_output:
		csv_output = csv.DictWriter(f_output, fieldnames =headers)
		f_output.seek(0,2)
		if f_output.tell() ==0:
			csv_output.writeheader()
   		csv_output.writerow({'Cell': number, 'AVG_Max': average_max, 'bck': background,'ER1':ER1, 'ER2':ER2, 'ER3':ER3, 'net_ER':net_ER, 'net_golgi':net_golgi, 'T_Index':t_index, 'Image_title':image_name})  
	getOptions(dest)
